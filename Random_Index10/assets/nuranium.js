        ...
        initFlickityTabsSlider: function (tabslider) {
            tabslider.each(function () {
                ...
                if (productGrid.not('.flickity-enabled')) {
                    ...
                    var itemlength = productGrid.find('.block').length;
                    const
                        numPages = Math.ceil(itemlength / item);
                        $(this).parent().find('.total_nav').text(`${numPages}`);

                    $(this).parent().find('.prev_custom').on( 'click', function() {
                      productGrid.flickity('previous');
                    });
                    $(this).parent().find('.next_custom').on( 'click', function() {
                      productGrid.flickity('next');
                    });
                    productGrid.on( 'change.flickity', function( event, index ) {
                      console.log( 'Flickity change ' + index );
                      var n = index + 1
                      $(this).parent().find('.current_nav').text(n);
                    });
                }
            });
        },
